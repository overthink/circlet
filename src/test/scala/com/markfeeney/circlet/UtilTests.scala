...
class UtilTests extends FunSuite {
  ...
  test("form decode to a map") {
    def decode(s: String): Map[String, String] = Util.formDecodeMap(s, UTF_8)
    assert(decode("") == Map.empty)
    assert(decode("a") == Map.empty)
    assert(decode("a=b") == Map("a" -> "b"))
    assert(decode("a=b&c=d%2fe") == Map("a" -> "b", "c" -> "d/e"))
    assert(decode("a=b&c=%zz") == Map("a" -> "b"))
    assert(decode("a=b&c") == Map("a" -> "b"))
    assert(decode("a=b&") == Map("a" -> "b"))
    assert(Util.formDecodeMap("a=foo%FE%FF%00%2Fbar", UTF_16) == Map("a" -> "foo/bar"))
  }
